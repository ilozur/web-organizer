...
class EditTodoForm(forms.Form):
    ...
    todo_edit_text = forms.CharField(label='Enter description', max_length=1000,
                                     widget=forms.Textarea(attrs={'form': 'add_todo_form', 'class': 'form-control', 'placeholder': 'Текст'}))
    todo_edit_time = forms.TimeField(widget=forms.TimeInput(attrs={'form': 'add_todo_form', 'class': 'form-control', 'type': 'time'}))
    todo_edit_deadline = forms.DateField(widget=forms.DateInput(attrs={'form': 'add_todo_form', 'class': 'form-control', 'type': 'date'}))
