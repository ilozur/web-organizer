...
class Event(models.Model):
    ...
    ##@brief Contain number of user who create this event.
    user = models.ForeignKey(User, default=1, on_delete=set([1, ]))

    ##@brief Contain date of adding event.
    added_date = models.DateField(default="2018-01-01")

    ##@brief Contain time of adding.
    added_time = models.TimeField(default="00:00:00:000000")

    ##@brief Contain title.
    title = models.CharField(max_length=255)

    ##@brief Contain description.
    description = models.TextField()

    ##@brief Contain informations about is an public this variable or not.
    is_public = models.BooleanField()

    ##@brief Contain date why this event will happen.
    date = models.DateField(default="2018-01-01")

    ##@brief Contain time why event will happen.
    time = models.TimeField()

    ##@brief Contain information about stage of progress of event.
    status = models.CharField(max_length=64, default="opened")

    ##@brief Contain information about sending notification to email (can be true or false).
    notified_already = models.BooleanField(default=False)

    ##@brief Contain information about time (hour) why notification will be send.
    should_notify_hours = models.SmallIntegerField(default=0)

    ##@brief Contain information about time (minute) why notification will be send.
    should_notify_minutes = models.SmallIntegerField(default=0)

    ##@brief Contain information about date why notification will be send.
    should_notify_days = models.SmallIntegerField(default=0)

    ##@brief Contain name of place where event will happen.
